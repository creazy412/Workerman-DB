...
//     file_put_contents('/receive.txt', $sss);
    ...
    if ( $data_socket['action'] ==  'reportposition'){
        ...
        $outMsg = reportPosition($data_socket, $peer);
    ...
    $peer_arr = explode(':', $peer);
    
    $udpConnectServer = udpGet("hello world!", $peer_arr[0], $peer_arr[1]);
    
    echo "request unicom server :".$udpConnectServer;
    
    file_put_contents('/tmp/udpserver.txt', json_encode($udpConnectServer));
...
//领队的gps位置信息
function reportPosition($data_socket, $peer){
    $dataBase = new DBCONNECT;
    
    $device_id = $data_socket['mifiid'];
    ...
    file_put_contents('/tmp/database.txt', json_encode($data));
    return json_encode(array('errcode'=>0, 'msg'=>'success', 'response'=>'leadermifipos', 'gpspons'=>$data, 'requester IP:'=>$peer));
...
function udpGet($sendMsg = '', $ip = '127.0.0.1', $port = '9998'){
    ...
    file_put_contents('/tmp/ip_port.txt', json_encode($ip));
    file_put_contents('/tmp/ip_port.txt', json_encode($port));
    $handle = stream_socket_client("udp://{$ip}:{$port}", $errno, $errstr);
    if( !$handle ){
        die("ERROR: {$errno} - {$errstr}\n");
        file_put_contents('/tmp/handle_error.txt', 'error');
    }
    fwrite($handle, $sendMsg."\n");
    $result = fread($handle, 1024);file_put_contents('/tmp/handle_result.txt', $result);
    fclose($handle);
    return $result;
}
